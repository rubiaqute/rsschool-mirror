...
import {
  showQuestion, eliminateModal, eliminateQuizPage, showCategories,
...
export default class Quiz {
  ...
  async createQuiz() {
    ...
    for (let i = 0; i < 4; i += 1) {
      const questionOptions = createNodetoDom('div', 'options');
      if (indexCategory < 12) questionOptions.innerText = options[i];
      else questionOptions.innerHTML = `<img src ="${options[i]}" alt = "">`;
      containerOptions.append(questionOptions);
  ...
  finishLevel() {
    eliminateModal();
    if (this.index > 11) showCategories('paintings');
    else showCategories('artists');
    writeResult(this.index);
    new Quiz(this.index).updateCategory();
  }
